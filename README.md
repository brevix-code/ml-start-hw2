...
Нужно предсказать значение `Transported`: был ли пассажир перенесён в другое измерение.
<b/>Выбрал этот датасет, потому что по сравнению с другими вариантами он имеет небольшой размер и понятные признаки. При этом датасет достаточно сложный для проверки feature engineering, нескольких моделей и ансамблей.
<b/>В обучающей выборке 8 693 пассажира и 14 столбцов, а в тестовой - 4 277 пассажиров и 13 столбцов. В данных нет полных дубликатов, но есть 2 324 пропущенных значения
